#!/usr/bin/env python3
"""El sobre comun de los archivos de veredicto del repositorio.

Un archivo de veredicto guarda lo que alguien LEYO sobre los candidatos que reporta un
...
Son seis archivos y cada uno se habia inventado su propia forma. La fecha del ultimo
repaso llego a llamarse de tres maneras --`revisado`, `_revisado`, `fijado`-- en tres archivos
que dicen lo mismo. Eso no rompe nada, pero son varias formas de leer mal un mismo concepto, y
el patron va a seguir apareciendo.

EL SOBRE, igual en los seis. `kb-procedencia.json` ya lo tenia y sirvio de modelo:

    _descripcion   que es y quien lo consume. Obligatorio.
    _criterio      por que existe y como se decide un veredicto. Opcional.
    _vocabulario   los valores validos del veredicto, con su significado. Opcional.
    fijado         fecha del ultimo repaso, ISO. Obligatorio.
...
pertenencia (`secuencias`, 774 entradas sin veredicto individual), un mapa de veredictos
(`leyes`, `lecturas`) y un mapa de listas (`revisiones`, porque una norma puede volver con mas
de un defecto). Forzar las tres a `items: {clave: {veredicto, fecha}}` inflaria la primera diez
veces y registraria 774 veredictos que nadie tomo. Un formato unico que miente sobre el
contenido es peor que tres formatos que lo dicen.

`argentina/fuentes/normas/revisiones.json` usa el mismo sobre pero NO pasa por aca: vive dentro
...
class SobreInvalido(ValueError):
    """El archivo no trae el sobre comun: se dice cual falta, no se adivina un default."""


def cargar(ruta: pathlib.Path, carga: str, vacio=None):
    """Devuelve (sobre, contenido). Si el archivo no esta, contenido es `vacio`.

    No se tolera un sobre incompleto en un archivo que SI existe: un veredicto sin fecha no
    se puede envejecer, y uno sin descripcion no se sabe quien lo consume.
    """
    if not ruta.exists():
