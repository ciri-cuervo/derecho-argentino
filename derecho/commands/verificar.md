...
Tarda unos minutos: son 55 normas y las pide de a una.

```!
python3 ${CODEX_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/fuentes/scripts/verificar_normas.py $ARGUMENTS 2>&1 | tail -70 || true
```
...
- **Alguna cambió** → esto es lo importante y hay que tratarlo como tal. Un hash distinto
  ...
  `python3 ${CODEX_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/fuentes/scripts/descargar_normas.py --slug <slug> --forzar`
...
- **Errores de red** → distinguilos de los cambios reales. Un timeout no es una reforma. Si
  hay varios, sugerí `python3 ${CODEX_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/fuentes/scripts/diagnostico.py`.
