...
class TestCuerpoContraProcedencia(unittest.TestCase):
    """El `.txt` que publicamos tiene que ser el texto que produjo el hash registrado.

    `verificar_normas.py` compara la descarga fresca contra `procedencia.json` y nunca abre el
    archivo del disco, así que nada cruzaba las dos cosas que el repositorio publica juntas: el
    texto y su hash. Un archivo editado a mano, o escrito por una corrida que no llegó a
    registrar, pasaba en verde para siempre.
...
class TestHtmlDeJurisprudenciaContraProcedencia(unittest.TestCase):
    """El `.html` de un fallo tiene que ser lo que dice `procedencia.json` que se guardó.

    De la página se guarda sólo el `<div class="contenido">`, así que hay DOS hashes y no uno:
    `sha256` es el de la página que sirvió el portal -- sirve para detectar que cambió -- y
    `sha256_guardado` es el del archivo en disco, que es lo único cotejable sin salir a la red.
    Nada cruzaba lo segundo, y la jurisprudencia no tiene verificador propio.
    """
    ...
    def test_cada_html_hashea_a_lo_que_dice_procedencia(self):
        import hashlib
        mirados, mal = 0, []
        for slug, reg in sorted(self.proc.items()):
            ...
            if not reg["archivo"].endswith(".html") or not archivo.is_file():
                continue
            mirados += 1
            esperado = reg.get("sha256_guardado")
            if not esperado:
                mal.append(f"{slug} (falta sha256_guardado)")
        ...
        self.assertEqual(mal, [],
                         f"el .html no hashea a su sha256_guardado: {', '.join(mal)}. "
                         f"Volver a bajar ese fallo con "
                         f"`descargar_jurisprudencia.py --forzar --slug <slug>`")

    def test_lo_guardado_es_el_fragmento_y_no_la_pagina(self):
        """El archivo empieza y termina en el div, sin la maqueta alrededor.

        No se controla la AUSENCIA de etiquetas de página: JUBA emite la sentencia envuelta en
        un `<html>` suelto adentro de un `<p>`, así que eso es carga y no maqueta. Lo que se
        ...
        for slug, reg in sorted(self.proc.items()):
            ...
            if not reg["archivo"].endswith(".html") or not archivo.is_file():
                continue
            if not reg.get("sha256_guardado"):
                continue            # lo reclama el test de arriba
            with self.subTest(slug):
