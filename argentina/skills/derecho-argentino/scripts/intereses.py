#!/usr/bin/env python3
"""Actualizacion e intereses sobre un credito - derecho argentino.

Calculadora determinista para la skill `derecho-argentino`. NO trae series: los indices se
leen de `argentina/fuentes/datos/serie-<nombre>.csv` y las tasas se pasan como entrada. Si la
serie no esta cargada, el script no estima: emite el marcador y corta.

Dos modos, que corresponden a dos regimenes distintos. Elegir el modo NO es una decision
tecnica: depende de que norma o doctrina rige el credito. Ver `references/laboral.md` 5.5
(fuero nacional) y 5.5 bis (PBA), y `references/sede-judicial-pba.md` 1.6.8.

    # Actualizacion por indice + interes puro (esquema tipo "Barrios", o art. 276 LCT)
...
def main():
    ...
    p.add_argument("--interes-puro", type=Decimal, default=Decimal("6"),
                   help="Interes puro anual. 'Barrios' fija un TECHO del 6%%, no una tasa fija")
    ...
    print("ACTUALIZACION E INTERESES\n")
    ...
    if a.modo == "indice":
        ...
        if not serie:
            print(f"\n  La serie '{a.serie}' no esta cargada. No se estima.\n")
            if datos(a.repo) is None:
                print("    No hay repo configurado en esta maquina: correr configurar.py")
            print(f"    [VERIFICAR MONTO ACTUALIZADO: serie {a.serie.upper()} para el periodo "
                  f"{p0} a {p1} - cargar argentina/fuentes/datos/serie-{a.serie}.csv desde la "
        ...
        marcadores.append(
            ...
            "la publicacion oficial del organismo a la fecha del calculo]")
        if a.interes_puro > 6:
            marcadores.append(
                "[VERIFICAR CRITERIO DEL FUERO: interes puro superior al 6% anual - "
                '"Barrios" (SCBA, C. 124.096, 17/4/2024) lo fija como techo]')
    else:
        if a.tna is None:
        ...
        marcadores.append(
            ...
            "cada tramo del periodo; este calculo usa una tasa unica y sin capitalizacion]")

    marcadores.append(
        "[VERIFICAR TASA VIGENTE: fuero laboral PBA - determinar si el credito cae bajo el "
        "art. 12 LRT texto Ley 27.348 (\"Galarza\", SCBA L. 132.729, 30/3/2026), bajo el "
        "art. 55 de la Ley 27.802, o bajo la doctrina \"Barrios\"]")

    print("\n  MARCADORES")
    for m in marcadores:
        print(f"    {m}")
    print("\n  Este script calcula un tramo unico. Si el credito tiene tramos con criterios "
          "distintos\n  (por ejemplo, hasta la cuantificacion y desde ahi al pago), correrlo "
          "una vez por tramo.")
